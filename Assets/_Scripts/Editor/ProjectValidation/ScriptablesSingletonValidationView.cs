...
namespace MetroidvaniaEditor.Validation.Views
{
    ...
    {
        class AssetHandler : AssetPostprocessor
        {
            static event System.Action<string> e_AssetImported;
            static event System.Action<string> e_AssetDeleted;
            static event System.Action<string, string> e_AssetMoved;
            ...
            public AssetHandler()
            {
                var guids = AssetDatabase.FindAssets($"t:{typeof(ScriptableSingleton<>).Name}");
            ...
            {
                if (singletonsPath.TryGetValue(path, out var deletedType))
                {
            ...
            {
                if (singletonsPath.TryGetValue(oldPath, out var movedType))
                {
            ...
            static bool IsSubclassOfRawGeneric(Type toCheck, Type baseType)
            {
        ...
        public ScriptablesSingletonValidationView(ProjectValidatorWindow root) : base(root)
        {
            ...
            {
                var element = _itemObjectTemplate.Instantiate();
        ...
        {
            var path = EditorUtility.SaveFilePanel($"Select {type.Name} singleton path", "Assets/Data/Settings", type.Name, "asset");
        ...
        {
            var instances = _assetHandler.singletonsPath.Where(pair => pair.Value == type)
        ...
        private InstanceStatus GetInstanceStatus(System.Type type)
        {
            ...
            if (instancesCount > 1) return InstanceStatus.Multiple;
            if (instancesCount == 1) return InstanceStatus.Single;
