...
namespace MetroidvaniaEditor.Validation
{
    public class ProjectValidatorWindow : EditorWindow
    {
        ...
        public static void ShowWindow()
        {
            ...
            var window = GetWindow<ProjectValidatorWindow>();
            window.Show();
        }
        ...
        {
            var visualTree = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(k_VisualTreeAssetPath);
            ...
            for (int i = 0; i < viewTypes.Count; i++)
            {
                ...
                var viewInstance = System.Activator.CreateInstance(viewType, viewConstructorParams) as ProjectValidatorView;
        ...
        {
            if (_selectedView != null && viewIndex == _selectedViewIndex) return;
